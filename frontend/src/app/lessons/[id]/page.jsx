...
//       try {
//         setLoading(true)
//         const response = await apiCall('http://localhost:5001/api/modules')
//         if (response.success) {
...
//               <Typography variant="body2" className="opacity-90">Total Modules</Typography>
//               <Typography variant="h5" className="font-bold">{modules.length}</Typography>
//             </Card>
//             <Card className="p-4 bg-gradient-to-r from-green-500 to-green-600 text-white">
//               <Typography variant="body2" className="opacity-90">Completed</Typography>
//               <Typography variant="h5" className="font-bold">{completedCount}</Typography>
//             </Card>
//             <Card className="p-4 bg-gradient-to-r from-purple-500 to-purple-600 text-white">
//               <Typography variant="body2" className="opacity-90">Total XP Earned</Typography>
//               <Typography variant="h5" className="font-bold">{totalXP}</Typography>
//             </Card>
//           </div>
//         </Container>
//       </header>
...
//                   <MenuItem key={difficulty} value={difficulty} className="capitalize">
//                     {difficulty}
//                   </MenuItem>
//                 ))}
//               </Select>
...
//             const CategoryIcon = categoryIcons[module.category] || BookIcon
...
//             return (
//               <Grid item xs={12} sm={6} lg={4} key={module._id}>
...
//                           size="small"
//                           className={`${difficultyColor.bg} ${difficultyColor.text} capitalize`}
//                         />
//                         <Chip
//                           label={status.replace('_', ' ')}
//                           size="small"
//                           className={`${statusColor.bg} ${statusColor.text} capitalize`}
//                         />
//                       </div>
//                     </div>

//                     {/* Content */}
//                     <Typography variant="h6" component="h3" className="font-semibold mb-2 line-clamp-2">
//                       {module.title}
//                     </Typography>
...
//                           <span>Progress</span>
//                           <span>{Math.round((userProgress.timeSpent / module.estimatedTime) * 100)}%</span>
//                         </div>
//                         <LinearProgress
//                           variant="determinate"
//                           value={Math.min((userProgress.timeSpent / module.estimatedTime) * 100, 100)}
//                           className="rounded"
//                         />
//                       </div>
//                     )}
...
export default function LessonsPage() {
  ...
  useEffect(() => {
    const fetchModules = async () => {
      try {
        ...
        if (response.success) {
          // Sort modules by order
          const sortedModules = response.modules.sort(
            (a, b) => a.order - b.order,
          );
          setModules(sortedModules);
        }
  ...
  const filteredModules = modules.filter((module) => {
    ...
    const matchesCategory =
      !categoryFilter || module.category === categoryFilter;
    const matchesDifficulty =
      !difficultyFilter || module.difficulty === difficultyFilter;
    const matchesStatus =
      !statusFilter ||
      (module.userProgress
        ? module.userProgress.status === statusFilter
        ...
        >
          {filteredModules.map((module, index) => {
            ...
            const progressPercentage =
              status === "in_progress" && userProgress
                ? Math.round(
                    (userProgress.timeSpent / module.estimatedTime) * 100,
                  )
                : 0;
                ...
                <div className="p-6 flex-1">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <span
                        className={`${difficultyColor} px-2 py-1 rounded-full text-xs font-medium capitalize`}
                      >
                        {module.difficulty}
                      </span>
                      <span
                        className={`${statusColor} px-2 py-1 rounded-full text-xs font-medium capitalize`}
                      >
                        {status.replace("_", " ")}
                      </span>
                    </div>
