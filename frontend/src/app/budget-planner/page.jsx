...
export default function BudgetPlannerPage() {
  ...
  useEffect(() => {
    if (budgetData) {
      setFormMonth(
        budgetData.month || new Date().toISOString().substring(0, 7),
      );
      setFormIncome(
    ...
    ? [
        {
          ...
          actual:
            ...
            (expensesByCategory["Books"] || 0) +
            (expensesByCategory["Housing"] || 0) +
            (expensesByCategory["Utilities"] || 0) +
            (expensesByCategory["Healthcare"] || 0),
        },
        {
          category: "Wants (30%)",
          ...
          actual:
            ...
            (expensesByCategory["Clothing"] || 0) +
            (expensesByCategory["Shopping"] || 0),
        },
        {
          category: "Savings (20%)",
  ...
  const handleSaveBudget = async () => {
    try {
      ...
      const monthToFetch =
        payload.month || new Date().toISOString().substring(0, 7);
      const budgetResponse = await getBudgetByMonth(monthToFetch).catch(
        async () => {
      ...
      setError(
        err.response?.data?.message || "Invalid JSON or failed to save budget",
      );
      setSuccessMessage("");
    } finally {
  ...
  const removeIncomeRow = (index) =>
    setFormIncome((prev) => prev.filter((_, i) => i !== index));
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      <div className="max-w-6xl mx-auto p-8">
        ...
        >
          ...
          <motion.div variants={itemVariants} whileHover="hover">
            <div
              className={`text-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-shadow ${
                balance >= 0
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <p
                    className={`text-sm font-medium ${balance >= 0 ? "text-blue-100" : "text-orange-100"}`}
                  >
                    Balance
                  </p>
                  <p className="text-2xl font-bold">
                ...
                <DollarSign
                  className={`w-8 h-8 ${balance >= 0 ? "text-blue-200" : "text-orange-200"}`}
                />
              </div>
            </div>
          </motion.div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <Typography
                      variant="h4"
                      className={`font-bold ${
                        budgetData.health.healthScore === "Excellent"
                          ? "text-green-600"
                          : budgetData.health.healthScore === "Good"
                            ? "text-blue-600"
                            : budgetData.health.healthScore === "Fair"
                              ? "text-yellow-600"
                              : "text-red-600"
                      }`}
                    >
        ...
        <Paper
          elevation={0}
          className="p-2 mb-6 dark:bg-gray-900/80 dark:text-white"
        >
          <Tabs
            value={tabValue}
                      ...
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-3">
                            <Typography
                              variant="body1"
                              className="font-medium pl-2 dark:text-white"
                            >
                              {transaction.description}
                            </Typography>
                          </div>
