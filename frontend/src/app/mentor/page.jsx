  ...
  useEffect(() => {
    const loadHistory = async () => {
      try {
        ...
        if (
          historyRes?.success &&
          Array.isArray(historyRes.conversation) &&
          historyRes.conversation.length > 0
        ) {
  ...
  const handleQuickTip = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          id: `tip-${Date.now()}`,
          content: "Failed to fetch a quick tip.",
          sender: "ai",
          timestamp: new Date(),
        },
      ]);
  ...
  const handleAnalyzeBudget = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          id: `analysis-${Date.now()}`,
          content: "Failed to analyze your budget.",
          sender: "ai",
          timestamp: new Date(),
        },
      ]);
  ...
  const handleLearningPath = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          id: `recs-${Date.now()}`,
          content: "Failed to fetch learning recommendations.",
          sender: "ai",
          timestamp: new Date(),
        },
      ]);
