...
const QnAPage = () => {
  ...
  const handleAskQuestion = async (e) => {
    ...
    if (!user) {
      setFormError("Please login to ask a question");
      return;
    }
    ...
    if (!questionForm.content.trim()) {
      setFormError("Question details are required");
      return;
    }

    try {
      setFormSubmitting(true);
      const tagsArray = questionForm.tags
        .split(",")
        .map((tag) => tag.trim())
        .filter((tag) => tag);
      ...
      if (response && response.success === false) {
        setFormError(response.message || "Failed to create question");
        return;
      }
    ...
    >
      <div className="flex gap-4">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          >
            <ThumbsUp
              className={`h-5 w-5 ${question.upvotedBy?.includes(user?.id) ? "text-green-500" : "text-gray-400"}`}
            />
          </button>
          ...
          >
            <ThumbsDown
              className={`h-5 w-5 ${question.downvotedBy?.includes(user?.id) ? "text-red-500" : "text-gray-400"}`}
            />
          </button>
        </div>
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-300 mt-2">
                  Ask questions, share knowledge, and help others learn about
                  finance
                </p>
              </div>
              <motion.button
          ...
          >
            ...
            {formError && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="mb-4 p-3 bg-red-100 dark:bg-red-900 border border-red-300 dark:border-red-700 text-red-700 dark:text-red-200 rounded-lg text-sm"
              >
                {formError}
              </motion.div>
            )}
            ...
            <form onSubmit={handleAskQuestion} className="space-y-4">
              ...
              <div className="flex justify-end gap-4 pt-4">
                ...
                >
                  ...
                  {formSubmitting ? "Submitting..." : "Ask Question"}
                </button>
              </div>
            </form>
          </motion.div>
